...
The minimum value is 518400 (6 months).`,
	...
	Flags: []cli.Flag{
		...
		&cli.BoolFlag{
			Name:  "increment-deal",
			Usage: "show whether the file in this deal is a file increnent",
			Value: false,
		},
		&cli2.CidBaseFlag,
	},
	Action: func(cctx *cli.Context) error {
		...
		//we now use VerifiedDeal to imply whether the deal is a increment or not
		if cctx.IsSet("increment-deal") {
			incrementDealParam := cctx.Bool("increment-deal")
			isVerified = incrementDealParam
		} else {
			isVerified = false
		}
...
uiLoop:
	for {
		...
		case "verified":
			...
			state = "increment"
		case "increment":
			afmt.Print("\nDoes this a file increment deal? (yes/no): ")
